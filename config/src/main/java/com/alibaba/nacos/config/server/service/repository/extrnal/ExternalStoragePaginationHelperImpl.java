...
class ExternalStoragePaginationHelperImpl<E> implements PaginationHelper {
    ...
    public Page<E> fetchPage(final String sqlCountRows, final String sqlFetchRows, final Object[] args,
        ...
        if (!isDerby()) {
            selectSql = translateLimit(selectSql);
        }
    ...
    public void updateLimit(final String sql, final Object[] args) {
        ...
        if (isDerby()) {
            ...
            sqlUpdate = translateUpdateLimit(sqlUpdate);
        }
    ...
    private String translateLimit(String sql) {
        if (StringUtils.isBlank(PropertyUtil.getDbPaginationSql())) {
            return sql;
        }
        return sql.replaceAll("(?i)LIMIT \\?,\\?", PropertyUtil.getDbPaginationSql());
    }

    private String translateUpdateLimit(String sql) {
        if (StringUtils.isBlank(PropertyUtil.getDbUpdatePagination())) {
            return sql;
        }
        return sql.replaceAll("limit \\?", PropertyUtil.getDbUpdatePagination());
    }
}
