...
public class PropertyUtil implements ApplicationContextInitializer<ConfigurableApplicationContext> {
     ...
     * DB driver.
     */
    private static String dbDriverClass = "com.mysql.cj.jdbc.Driver";

    /**
     * db pagination sql regex.
     */
    private static String dbPaginationSql = "";

    /**
     * db update pagination sql regex.
     */
    private static String dbUpdatePagination = "";

    /**
     * db check readonly sql.
     */
    private static String dbCheckReadonlySql = "";

    /**
     * db test query sql.
     */
    private static String dbTestQuerySql = "";
    ...
    public static void setDbDriverClass(String dbDriverClass) {
        PropertyUtil.dbDriverClass = dbDriverClass;
    }

    public static String getDbPaginationSql() {
        return dbPaginationSql;
    }

    public static void setDbPaginationSql(String dbPaginationSql) {
        PropertyUtil.dbPaginationSql = dbPaginationSql;
    }

    public static String getDbCheckReadonlySql() {
        return dbCheckReadonlySql;
    }

    public static void setDbCheckReadonlySql(String dbCheckReadonlySql) {
        PropertyUtil.dbCheckReadonlySql = dbCheckReadonlySql;
    }

    public static String getDbUpdatePagination() {
        return dbUpdatePagination;
    }

    public static void setDbUpdatePagination(String dbUpdatePagination) {
        PropertyUtil.dbUpdatePagination = dbUpdatePagination;
    }

    public static String getDbTestQuerySql() {
        return dbTestQuerySql;
    }

    public static void setDbTestQuerySql(String dbTestQuerySql) {
        PropertyUtil.dbTestQuerySql = dbTestQuerySql;
    }
    ...
    private void loadSetting() {
        try {
            ...
            setDbCheckReadonlySql(getString("spring.datasource.check-readonly", " SELECT @@read_only "));
            setDbPaginationSql(getString("spring.datasource.pagination", ""));
            setDbUpdatePagination(getString("spring.datasource.update-pagination", ""));
            setDbDriverClass(getString("spring.datasource.driver-class-name", "com.mysql.cj.jdbc.Driver"));
            String dataSource = getString("spring.datasource.platform", "");
            LOGGER.info("spring.datasource.platform: {}", dataSource);
            setUseExternalDB(!StringUtils.isBlank(dataSource));
            setDbTestQuerySql(getString("spring.datasource.connection-test-query", "SELECT 1 FROM dual"));
